...
class Game:
    ...
    def _draw_units(self, visible):
        ...
        for unit in self.player_units + self.bot_units:
            ...
            if unit.is_player and player_unit_img:
                img_rect = player_unit_img.get_rect(center=(int(unit.px), int(unit.py)))
                self.screen.blit(player_unit_img, img_rect)
            elif not unit.is_player and bot_unit_img:
    ...
    def _draw_game_over(self):
        ...
        if big_font and font:
            ...
            self.screen.blit(winner_shadow, (WIDTH // 2 - 148, HEIGHT // 2 - 38))
            self.screen.blit(winner_text, (WIDTH // 2 - 150, HEIGHT // 2 - 40))

            restart_text = font.render("Press SPACE to restart", True, UI_TEXT)
            self.screen.blit(restart_text, (WIDTH // 2 - 100, HEIGHT // 2 + 30))
    ...
    def run(self):
        ...
        while running:
            ...
            interface_btn = draw_interface(
                ...
                self.bot_units, self.turn_timer_start, TURN_TIME
            )
